...
def _explain_error(question: str, correct_ans: str) -> str:
    q = question.lower()
    if "сумму" in q or "a + b" in q or "вычислите a + b" in q:
        return "Сложение матриц: складывайте элементы с одинаковыми позициями (i,j). Результат имеет тот же размер."
    if "скаляр" in q or "умножьте на" in q or "·a" in q:
        return "Умножение на скаляр: умножьте каждый элемент матрицы на данное число k."
    if "aᵀ" in q or "транспон" in q:
        return "Транспонирование: строки становятся столбцами. Элемент (i,j) переходит на позицию (j,i)."
    if "a⁻¹" in q or "обратн" in q:
        return "Обратная 2×2: A⁻¹ = (1/det)·[[d,−b],[−c,a]], где det = ad−bc. Проверьте знаки."
    if "2×2" in q and ("определ" in q or "det" in q):
        return "Определитель 2×2: det = a·d − b·c. Главная диагональ минус побочная."
    if "3×3" in q and ("определ" in q or "det" in q):
        return "Определитель 3×3: разложите по первой строке со знаками +−+. Каждый минор — определитель 2×2."
    if "произведение" in q or "a·b" in q:
        return "Умножение матриц: c_ij = сумма(a_ik·b_kj). Строка первой на столбец второй поэлементно."
    if "ранг" in q:
        return "Ранг: приведите матрицу методом Гаусса к ступенчатому виду, посчитайте ненулевые строки."
    if "систему" in q or "x₁" in q:
        return "Метод Крамера: x_i = det(Aᵢ)/det(A). Замените i-й столбец столбцом правых частей b."
    if "размер" in q:
        return "Размер записывается как «строки × столбцы». Сначала считайте горизонтальные строки."
